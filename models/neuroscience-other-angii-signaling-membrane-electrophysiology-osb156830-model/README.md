# 156830

**Source**: [opensourcebrain](https://github.com/OpenSourceBrain/156830)
**Standard**: other
**Authors**: Open Source Brain

## Description

Model of AngII signaling and membrane electrophysiology (Makadia, Anderson, Fey et al., 2015)


## Usage

This model was auto-generated from the opensourcebrain repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/neuroscience-other-angii-signaling-membrane-electrophysiology-osb156830-model/model.yaml
```

## Tags

other, opensourcebrain, auto-generated, neuroscience
